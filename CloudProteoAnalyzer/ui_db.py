...
def Downloadfromdrive(session_dict:dict,sipros_path:str,config_dict:dict,page_drive:st):
    ...
    for one_file in range(len(all_files)):
        ...
        if all_files_name[one_file].find(".raw") != -1:
            ...
            os.system("mono /home/jiancheng/Sipros_Pept/Raxport.exe -i "+session_dict['whole_path']+" -o "+session_dict['whole_path'])
...
def paramsTable(form:st, config_dict:dict):
    label_search = form.selectbox(label="Search type:(SIP: stable isotope probing. Regular: label free quantification.)",options=["Regular","SIP"])
...
def publicationPage(col22:st)->None:
    ...
    col22.write(f'''<p>1. Guo, X., Li, Z., Yao, Q., Mueller, R.S., Eng, J.K., Tabb, D.L., Hervey IV, W.J. and Pan, C., 2018. Sipros ensemble improves database searching and filtering for complex metaproteomics.<a href={url2}> Bioinformatics, 34(5), pp.795-802</a></p>''',unsafe_allow_html=True)
    col22.write(f'''<p>2. Wang, Y., Ahn, T.H., Li, Z. and Pan, C., 2013. Sipros/ProRata: a versatile informatics system for quantitative community proteomics.<a href={url3}> Bioinformatics, 29(16), pp.2064-2065</a></p>''',unsafe_allow_html=True)
    col22.write(f'''<p>3. Pan, C., Kora, G., McDonald, W.H., Tabb, D.L., VerBerkmoes, N.C., Hurst, G.B., Pelletier, D.A., Samatova, N.F. and Hettich, R.L., 2006. ProRata: a quantitative proteomics program for accurate protein abundance ratio estimation with confidence interval evaluation.<a href={url1}> Analytical chemistry, 78(20), pp.7121-7131</a></p''',unsafe_allow_html=True)
...
def demoPage(button_cond,col2)->None:
    col2.markdown("""
    <style>div[class="css-6vw1hb e1tzin5v2"] {height: 700px;} 
    div[class="css-1mapwt e1tzin5v0"] {height: 900px;}
    </style>
    ...
    if button_cond and "110618_yeast_ups_50fmol_r1.SE.ProRata_LabelFree_Peptide.txt" in os.listdir(data_path):
        for tmp in os.listdir(data_path):
            if tmp.find(".txt")!=-1:
                os.remove(os.path.join(data_path,tmp))
    new_col = col2.container()
    ...
    new_col.write()
    new_col.write()
    submit_button = new_col.button(label='Run Demo data')
    if submit_button:
        new_col.write()
        new_col.write()
        new_col.write()
        ...
        new_col.write()
        new_col.write()
        job_id=functions.Runonserver(host_inf,data_path,False,"sbatchscript.sh", True)
        statues_running = ""
        while True:
            try:
                check_status, running_time = functions.Checkstatus(host_inf,job_id,"scratch/jiancheng/example")
            except:
                print("sever error!")
                check_status="RUNNING"
                running_time=""
            ...
            time.sleep(150)
        if statues_running!="COMPLETED":
            new_col.write("Program running! Please waiting and not click any button!")         
            new_col.write("")  
            new_col.write("")                                
            new_col.write("")                                
            new_col.write("")                                
            ...
            functions.downloadDemoFile(host_inf)
            result=set()
            for fname in os.listdir(data_path):
                if fname.find(".FT1")!=-1:
                    result.add(fname.replace(".FT1",".raw"))
...
def loginPage(col22:st)->None:
    flow = google_auth_oauthlib.flow.Flow.from_client_secrets_file(
        # 'client_secret_63250039343-grd5u897a70pfc7j9mor3h11jdlhto6o.apps.googleusercontent.com.json',
        'client_secret_926147001357-hpij9refffful202iujebn5fdupfijg6.apps.googleusercontent.com.json',
        scopes = ['https://www.googleapis.com/auth/drive','https://www.googleapis.com/auth/userinfo.email'])  
    flow.redirect_uri = 'https://sipros.oscer.ou.edu'
