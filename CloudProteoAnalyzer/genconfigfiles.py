...
def generateSlurmOpenmp(path:str, params:dict, session_dict:dict)->None:
    with open(path,"w") as f:
        ...
        f.write("TabFile=$(python2 "+script_path+"/sipros_psm_tabulating.py -i result/ -c generated_sipros_config.cfg -o result/)\n")
        f.write("python2"+script_path+"/sipros_ensemble_filtering.py -i ${TabFile} -c generated_sipros_config.cfg -o result/\n")
        f.write("python2 "+script_path+"/sipros_peptides_assembling.py -w result/ -c generated_sipros_config.cfg\n")
        ...
        for tmp in files:
            ...
            # f.write("ProRata.linux -w result/ -c ProRataConfig_LabelFree.xml -i result/*.pro2psm.txt")#########################################
    f.close()
...
def generateSlrumMPI(path:str, path2:str, params:dict, session_dict:dict)->None:
    with open(path,"w") as f:
        ...
        f.write("export OMPI_MCA_btl_openib_allow_ib=1\n")
        f.write("export OMPI_MCA_btl_openib_if_include=\"mlx4_0:1\"\n")
        ...
        f.write("module load python/anaconda2-4.2.0\n")
        if functions.readUploadFile(os.path.join(user_data_path,session_dict['user_email']),"multiple")=="1":
            f.write("mpirun -np $SLURM_NTASKS /home/jiancheng/condaEnsembleBIN/SiprosEnsembleMPI -w " +"raw_data" + " -c generated_sipros_config.cfg -o result\n")           
        else:
            f.write("mpirun -np $SLURM_NTASKS /home/jiancheng/condaEnsembleBIN/SiprosEnsembleMPI -f " +params["ms file"] + " -c generated_sipros_config.cfg -o result\n")
        f.write("TabFile=$(python2 "+script_path+"/sipros_psm_tabulating.py -i result/ -c generated_sipros_config.cfg -o result/)\n")
        f.write("python2 "+script_path+"/sipros_ensemble_filtering.py -i ${TabFile} -c generated_sipros_config.cfg -o result/\n")
        f.write("python2 "+script_path+"/sipros_peptides_assembling.py -w result/ -c generated_sipros_config.cfg\n")
        if params["search type"]=="SIP":
            f.write("python2 "+script_path+"/ClusterSip.py -w result/ -c generated_sipros_config.cfg\n")
    f.close()
    with open(path2,"w") as f:
        ...
        files=functions.readUploadFile(os.path.join(user_data_path,session_dict['user_email']),"files").strip().split("~")
        for tmp in files:
            f.write("ProRata.linux -w "+tmp+"/ -c ProRataConfig.xml\n")
