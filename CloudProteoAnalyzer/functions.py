...
def sendEmail(message,session_dict,attach_label:bool):
    ...
    # port = 587  
    # sender_email = "licui110728@gmail.com"
    # password = "ytgtiwvvefolqjkj"
...
def Runonserver(host_inf:dict,session_dict:dict,del_label:bool,sbatch_config_file:str, again_label:bool):
    ...
    the_path = os.path.join(server_data_path,session_dict['whole_path'].split("/")[-2]+"+"+session_dict['whole_path'].split("/")[-1])
    if again_label:
        if not del_label:
...
def Downloadfile(host_inf:dict,session_dict:dict,search_type:str):
    ...
    the_path = os.path.join(server_data_path,session_dict['whole_path'].split("/")[-2]+"+"+session_dict['whole_path'].split("/")[-1])
    for tmp in files:
        _,tar_state,_=client.exec_command("ls "+the_path+"/"+tmp+"/")
        #print(tar_state.read().decode().strip())
        _,tar_state,_=client.exec_command("cp "+the_path+"/"+tmp+"/"+"*Peptide.txt "+the_path+"/result/"+tmp+".ProRata_LabelFree_Peptide.txt")
        _,tar_state,_=client.exec_command("cp "+the_path+"/"+tmp+"/"+"*Protein.txt "+the_path+"/result/"+tmp+".ProRata_LabelFree_Protein.txt")
        _,tar_state,_=client.exec_command("ls "+the_path+"/result/")
        tar_state.read().decode().strip()
        #print(tar_state.read().decode().strip())
    if search_type == "SIP":
        _,tar_state,_=client.exec_command("cd "+the_path+"/result"+" ; "+"tar -cvzf results.tar.gz *.txt ; chmod 777 results.tar.gz")

    else:
        #print(the_path+"/result/")
        #_,tar_state,bbb=client.exec_command("cd "+the_path+"/result")
        #print("aaaaa")
        #print(tar_state.read().decode().strip())
