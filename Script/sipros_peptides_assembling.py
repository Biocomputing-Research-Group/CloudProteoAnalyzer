...
"""
1. print out N/A if the protein description is not available
"""
...
Options:
    ...
    -d/--decoy-label decoy label   #decoy protein label
...
def parse_options(argv):
    global decoy_prefix
    try:
        opts, args = getopt.getopt(argv[1:], "hvVw:d:c:",
                                   ["help",
                                    "version",
                                    "working-dir",
                                    "decoy-label",
...
def parse_config(config_filename):
    ...
    all_config_dict = parseConfigKeyValues(config_filename)
...
def report_output(config_dict,
    ...
    def_para_msg = ""
    def_para_msg += "#\t########################################\n"
    def_para_msg += "#\t##### Peptide Assembling #####\n"
    def_para_msg += "#\t########################################\n"
    def_para_msg += "#\t\n"
    def_para_msg += "#\t###############\n"
    ...
    for protein_one in pro_greedy_list:
        ...
        for pep_file, run_num in sorted(run_num_dict.items(), key=lambda x: x[1][-1]):
            ...
            for pep_list_one in pep_list:
                ...
                if pep_run_id in pep_data_dict:
                    ...
                    for pep_pro_dict_item in pep_pro_dict[pep_list_one]:
                        share_protein_list.append(pep_pro_dict_item)
            ...
            # share_protein_list and num
            share_protein_list = list(set(share_protein_list))
            share_protein_number = len(share_protein_list)

            # check share protein num
            if share_protein_number != 0:
                balanced_spectrum_counts = divide(float(total_spectrum_counts), float(share_protein_number))
            else:
                balanced_spectrum_counts = 0.0
    ...
    for pro_out_data_idx, pro_out_data_item in enumerate(pro_out_data):
        ...
        for pep_file, run_num in sorted(run_num_dict.items(), key=lambda x: x[1][-1]):
            ...
            nbsc_idx = first_nbsc_idx + inc_int * (run_int - 1)  # calculate BalancedSpectrumCounts column index
    ...
    return protein_fdr, total_proteins_after_filtering


def adjust(FDR):
    PSMs = readData('test.data.txt','test.data.pin')
...
def main(argv=None):
    ...
    try:
        ...
        try:
            ...
            sys.stderr.write(
                '[%s] Beginning sipros_peptides_assembling.py run (V%s)\n' % (curr_time(), get_version()))
            sys.stderr.write('------------------------------------------------------------------------------\n')
            ...
            # Report output files
            FDR, target_pro = report_output(config_dict,
                                                run_num_dict,
                                                psm_run_num_dict,
                                                pep_data_dict,
                                                psm_data_dict,
